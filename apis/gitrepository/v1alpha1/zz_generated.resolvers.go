...
func (mg *GitRepository) ResolveReferences(ctx context.Context, c client.Reader) error {
	...
	rsp, err = r.Resolve(ctx, reference.NamespacedResolutionRequest{
		CurrentValue: mg.Spec.ForProvider.ParentRepositoryID,
		Extract:      GitRepositoryID(),
		Namespace:    mg.GetNamespace(),
		Reference:    mg.Spec.ForProvider.ParentRepositoryRef,
		Selector:     mg.Spec.ForProvider.ParentRepositorySelector,
		To: reference.To{
			List:    &GitRepositoryList{},
			Managed: &GitRepository{},
		},
	})
	if err != nil {
		return errors.Wrap(err, "mg.Spec.ForProvider.ParentRepositoryID")
	}
	mg.Spec.ForProvider.ParentRepositoryID = rsp.ResolvedValue
	mg.Spec.ForProvider.ParentRepositoryRef = rsp.ResolvedReference
